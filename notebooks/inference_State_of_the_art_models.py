...
from lib.metrics import jaccard, tversky, dice_coef, dice_loss, bce_dice_loss_new, focal_tversky_loss, create_dir, mcc_loss, mcc_metric
...
def show_visualization(df_pred, filename, model_dict, num_images):
    ...
    fig, axs = plt.subplots(num_images, num_models + 2, figsize=(8 * (num_models + 2), 8 * num_images)) # (width, height)
    W, H = 512, 512
    dim = (W,H)
    ...
    for count, row in enumerate(df_pred.itertuples()):
        if row.has_mask == 1 and count < num_images:
            ...
            axs[count, 0].set_title('Original Image ' + str(count + 1), fontsize=68, pad=6)
            ...
            for j, (model_name, model) in enumerate(model_dict.items()):
                ...
                img_model[pred == 1] = [ (0, 255, 0), (255, 255, 0), (204, 0, 204), (51, 51, 255),  (0, 255, 255), ][j] # (255, 255 , 255), (0, 255, 255), (127, 0, 255)
                ...
                axs[count, j + 2].set_title(f'[IoU B{j}={iou:.2f}]', fontsize=68, pad=6)
...
path_test_levee = os.path.join(root_dir,  "datasets", "test_images", "with_grassland") #with_grassland, #no_grassland inside "test_images", pothole_test exp2_test
print(path_test_levee)

test_images_leveecrack = (next(os.walk(path_test_levee + "/images"))[2])

random.Random(42).shuffle(test_images_leveecrack)

model_root_path = os.path.normpath(os.getcwd() + os.sep + os.pardir)
model_path = os.path.join(model_root_path, "models", "IEEE_Access_Models_trained_on_6853images", "sandboilnet_variations_4e_4") #state_of_the_art_models_4e_4 #sandboilnet_variations_4e_4 #baseline_comparision_models_4e_4
...
# M0 = load_model(model_path, 'unet_bce_dice_loss_new', False)
# M1 = load_model(model_path,'multiresunet_bce_dice_loss_new', False)
# M2 = load_model(model_path,'attentionunet_bce_dice_loss_new', False)
# M3 = load_model(model_path,'nestedunet_bce_dice_loss_new', False)
# M4 = load_model(model_path,'SandBoilNet_Low_Dimension_PCA_bce_dice_loss_new', True)

# M0 = load_model(model_path, 'Baseline_Conv_bce_dice_loss_new', False)
# M1 = load_model(model_path,'Baseline_LeakyRI_bce_dice_loss_new', False)
# M2 = load_model(model_path,'Baseline_CBAM_bce_dice_loss_new', False)
# M3 = load_model(model_path,'Baseline_SE_bce_dice_loss_new', False)
# M4 = load_model(model_path,'Baseline_ProposedAtt_bce_dice_loss_new', False)

M0 = load_model(model_path, 'baseline_normal_bce_dice_loss_new', False)
M1 = load_model(model_path,'SandBoilNet_CBAM_bce_dice_loss_new', True)
M2 = load_model(model_path,'SandBoilNet_SE_bce_dice_loss_new', True)
M3 = load_model(model_path,'SandBoilNet_Dropout_Without_PCA_bce_dice_loss_new', False)
M4 = load_model(model_path,'SandBoilNet_Low_Dimension_PCA_bce_dice_loss_new', True)
...
df_for_visualization = df_all_preds_all.sample(frac = 0.96)
show_visualization(df_for_visualization, "comparision_no_cbam_se_att_grassland.png", model_dict, num_images)
