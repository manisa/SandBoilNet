    ...
    for count, row in enumerate(df_pred.itertuples()):
        if row.has_mask == 1 and count < num_images:
            ...
            for j, (model_name, model) in enumerate(model_dict.items()):
                ...
                img_model[pred == 1] = [ (0, 255, 0), (255, 255, 0), (204, 0, 204), (51, 51, 255),  (0, 255, 255), (120, 120, 120) ][j] #(255, 255 , 255),
                ...
                axs[count, j + 2].set_title(f'[IoU IM{j + 1}={iou:.2f}]', fontsize=68, pad=6)
...
def load_model(model_name, custom_layer=False):
    ...
    root_dir = os.path.normpath(os.getcwd() + os.sep + os.pardir)
    model_path = os.path.join(root_dir,  "models", "models_trained_with_20percValid_4e_4", str(model_name), "best_model.h5")
...
path_test_levee = os.path.join(root_dir,  "datasets", "test_images", "test") #with_grassland, #no_grassland inside "test_images", pothole_test
print(path_test_levee)

test_images_leveecrack = sorted(next(os.walk(path_test_levee + "/images"))[2])

random.Random(2021).shuffle(test_images_leveecrack)
...
# M0 = load_model('Baseline_Normal_23_bce_dice_loss_new', False)
# M1 = load_model('SandBoilNet_CBAM_512_bce_dice_loss_new', True)
# M2 = load_model('SandBoilNet_SE_512_bce_dice_loss_new', True)
# M3 = load_model('SandboilNet_Dropout_Without_PCA_512_bce_dice_loss_new', True)
...
M4 = load_model('SandBoilNet_Low_Dimension_PCA_20_bce_dice_loss_new', True)
M5 = load_model('Depthwise_Seepage_Inception_PCA_20_bce_dice_loss_new', True)

# M0 = load_model('Baseline_Conv_512_bce_dice_loss_new', False)
# M1 = load_model('Baseline_LeakyRI_512_bce_dice_loss_new', True)
# M2 = load_model('Baseline_CBAM_512_bce_dice_loss_new', True)
# M3 = load_model('Baseline_SE_512_bce_dice_loss_new', True)
# M4 = load_model('Baseline_Att_512_bce_dice_loss_new', True)
...
show_visualization(df_all_preds_all, "test_comparision.png", model_dict, num_images)
